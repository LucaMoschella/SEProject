...
Un utente non autenticato può effettuare un’iscrizione o l’autenticazione (\bloccofunref{bloccofun:iscAut}), può effettuare ricerche (\bloccofunref{bloccofun:ric}), può consultare notizie (\funref{fun:vis.not}), può vedere prodotti e notizie simili a quelle che sta visualizzando (\funrefs{fun:prod.sim}{fun:not.sim}) e può inoltre visualizzare tutte le sezioni pubbliche del sito (\bloccofunref{bloccofun:gestvis}).

\subsection{Produttore autenticato}
Un produttore autenticato può gestire lo spazio dedicato alla presentazione prodotti (\bloccofunref{bloccofun:gestvet}), può effettuare ricerche (\bloccofunref{bloccofun:ric}), può segnalare contenuti e commentare recensioni (\funrefs{fun:comm.rec}{fun:report.cont}), può consultare notizie (\funref{fun:vis.not}), può usufruire del sistema di follower/followed (\bloccofunref{bloccofun:gestfoll}), può usufruire delle funzionalità di gestione account (\bloccofunref{bloccofun:gestacc}) e può inoltre visualizzare tutte le sezioni pubbliche del sito (\bloccofunref{bloccofun:gestvis}).

\subsection{Utente autenticato}
Un utente autenticato può effettuare ricerche (\bloccofunref{bloccofun:ric}), può effettuare qualsiasi operazione sulle recensioni tranne l’eliminazione e la modifica delle recensioni di altri utenti (\bloccofunref{bloccofun:gestrec} tranne le \funrefs{fun:del.recAltrui}{fun:mod.recAltrui}),  può consultare notizie (\funref{fun:vis.not}), può usufruire del sistema di follower/followed  (\bloccofunref{bloccofun:gestfoll}), può usufruire del sistema di suggerimenti automatici  (\bloccofunref{bloccofun:gestsugg}), può usufruire delle funzionalità di gestione account (\bloccofunref{bloccofun:gestacc}) e può anche visualizzare tutte le sezioni pubbliche del sito  (\bloccofunref{bloccofun:gestvis}).
...
\subsection{Newser}
Si tratta di un utente autenticato (quindi ne eredita tutte le funzionalità), ma ha in più la completa gestione delle news (\bloccofunref{bloccofun:gestnot}).

\subsection{Support}
Si tratta di un utente autenticato (quindi ne eredita tutte le funzionalità), ma ha in più la gestione delle iscrizioni con approvazione (\bloccofunref{bloccofun:gestisc}).

\subsection{Moderatore}
Si tratta di un utente autenticato (quindi ne eredita tutte le funzionalità), ma ha in più la gestione totale del sistema recensioni (\bloccofunref{bloccofun:gestrec}).
